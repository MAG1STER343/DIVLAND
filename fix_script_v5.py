...
if os.path.exists(path):
    ...
    # 3. Remove all instances of renderShop
    ...
    # 4. Remove all instances of let me = null;
    content = content.replace('let me = null;', '')
    
    # 5. Global Cleanup of stray residues
    content = content.replace('initGlobalUI();', '')
    content = content.replace('L`;', '').replace(')` : \'none\';', '')

    # 6. Definitions to Inject
    injection = '''
  let me = null;
...
'''
    ...
    # Inject into IIFE after transitioning flag
    marker = 'let transitioning = false;'
    content = content.replace(marker, marker + injection)

    with open(path, 'w', encoding='utf-8') as f:
        f.write(content)
    print("Script repaired ATOMICALLY at IIFE start.")
else:
